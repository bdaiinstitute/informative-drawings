...
import numpy as np
from informative_drawings.utils import channel2width
...
with torch.no_grad():
    ...
    if opt.reconstruct == 1:
        ...
        net_GB.load_state_dict(
            torch.load(
                os.path.join(opt.checkpoints_dir, opt.name, "netG_B_%s.pth" % opt.which_epoch), map_location=device
            )
        )
        net_GB.eval()
    ...
    if opt.predict_depth == 1:
        ...
        net_recog = InceptionV3(
            opt.num_classes, False, use_aux=True, pretrain=True, freeze=True, every_feat=opt.every_feat == 1
        )
        net_recog.to(device)
        net_recog.eval()
    ...
    net_G.load_state_dict(
        torch.load(os.path.join(opt.checkpoints_dir, opt.name, "netG_A_%s.pth" % opt.which_epoch), map_location=device)
    )
    ...
    test_data = UnpairedDepthDataset(
        opt.dataroot, "", opt, transforms_r=transforms_r, mode=opt.mode, midas=opt.midas > 0, depthroot=opt.depthroot
    )
